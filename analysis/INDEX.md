...
| `injector_diff.md` | **Diff between SakuraTools `injector/` and `data/GarlicInjector.exe`.** 大蒜 added: hex‑decode CLI, FNV‑1a verifier, PBKDF2 + AES‑GCM decryption layer, 24‑byte auth block injection, Themida protection. The `injector/` files are verbatim SakuraTools; `GarlicInjector.exe` is a wrapper around them. |
| `injector_chain.md` | **The full auth chain: launcher → injector → DLL.** Mermaid sequence diagram + the 5 connection points (FNV-1a nonce generation, hex-decode, remote-memory auth block write, CreateRemoteThread lpParameter handoff, ReflectiveLoader arg1 forwarding to DllMain). Also documents what happens if you delete the chain.
